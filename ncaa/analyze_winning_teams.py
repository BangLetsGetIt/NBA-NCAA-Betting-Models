...
TRACKING_FILE = 'ncaa/ncaab_picks_tracking.json'
...
def analyze_teams():
    ...
    # Output Winners
    print("\n" + "="*80)
    print("🏀 NCAA TEAMS TO ALWAYS BET ON (TOP 25) 🏀")
    print("="*80)
    ...
    for team, stats in sorted_teams[:30]:
        ...
        if total < 3: continue # Filter low sample size
        
        print(f"{team:<35} | {net_units:>+9.1f}u | {record:<10} | {win_pct:.0f}%")

    # Output Losers (Bonus)
    print("\n" + "="*80)
    print("🛑 TEAMS TO AVOID (TOP 10 LOSERS) 🛑")
    print("="*80)
