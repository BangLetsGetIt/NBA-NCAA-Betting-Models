...
MIN_EDGE_OVER_LINE = 15.0  # Player must average 25+ above prop line for OVER
MIN_EDGE_UNDER_LINE = 12.0  # Player must average 20+ below prop line for UNDER
...
def calculate_probability_edge(ai_score, season_avg, recent_avg, prop_line, odds, bet_type):
    ...
    # For rushing yards, normalize edge differently (25 yards = full edge factor)
    edge_factor = min(abs(edge) / 15.0, 1.0)
    
    recent_factor = 0.0
    if bet_type == 'over' and recent_avg > season_avg:
        recent_factor = min((recent_avg - season_avg) / 15.0, 0.1)
    elif bet_type == 'under' and recent_avg < season_avg:
        recent_factor = min((season_avg - recent_avg) / 15.0, 0.1)
                ...
                if 'bookmakers' in odds_data and odds_data['bookmakers']:
                    ...
                    if 'markets' in fanduel:
                        for market in fanduel.get('markets', []):
                            if market.get('key') == 'player_rush_yds':
...
def calculate_ai_score(player_data, prop_line, bet_type, opponent_defense=None):
    ...
    carry_share = player_data.get('carry_share', 0)
    ...
    if carry_share < 0.15:  # Not enough volume
        return 0.0
    ...
    if bet_type == 'over':
        ...
        if edge_above_line >= MIN_EDGE_OVER_LINE:
            score += 3.5
        elif edge_above_line >= 10.0:
            score += 2.5
        elif edge_above_line >= 7.0:
            score += 1.5
        elif edge_above_line >= 3.0:
            score += 0.5
        else:
            score -= 2.0
            if recent_avg < prop_line + 5.0:
                return 0.0

        recent_edge = recent_avg - prop_line
        if recent_edge >= MIN_RECENT_FORM_EDGE:
            score += 2.5
        elif recent_edge >= 7.0:
            score += 1.5
        elif recent_avg > season_avg + 10.0:
            score += 1.0
        ...
        if edge_below_line >= MIN_EDGE_UNDER_LINE:
            score += 3.5
        elif edge_below_line >= 10.0:
            score += 2.5
        elif edge_below_line >= 7.0:
            score += 1.5
        elif edge_below_line >= 3.0:
            score += 0.5
        else:
            score -= 2.0
            if recent_avg > prop_line - 5.0:
    ...
    if bet_type == 'over' and season_avg < prop_line + 5.0:
        final_score = min(final_score, 8.5)
    elif bet_type == 'under' and season_avg > prop_line - 5.0:
        final_score = min(final_score, 8.5)
...
def calculate_ev(ai_score, prop_line, season_avg, recent_avg, odds, bet_type):
    ...
    # Normalize edge for rushing yards (25 yards = full factor)
    edge_factor = min(abs(edge) / 15.0, 1.0)
...
def generate_html_output(over_plays, under_plays, tracking_summary=None, tracking_data=None):
    ...
    if tracking_summary and tracking_summary['total'] > 0:
        ...
        if True:
            ...
            tracking_section = f"""
            <div class="card">
                <h2 style="font-size: 1.75rem; font-weight: 700; margin-bottom: 1.5rem; text-align: center;">📊 NFL Rushing Yards Model Tracking</h2>
...
<head>
    ...
    <title>NFL Rushing Yards Props - A.I. Projections</title>
    <style>
