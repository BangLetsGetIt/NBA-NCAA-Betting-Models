#!/usr/bin/env python3
"""
NFL Rushing Yards Props Model - PROFITABLE VERSION
Analyzes player rushing yards props using REAL NFL stats
Focuses on carry share, opponent run defense, and matchup advantages
"""
...
OUTPUT_HTML = os.path.join(SCRIPT_DIR, "nfl_rushing_yards_props.html")
TRACKING_FILE = os.path.join(SCRIPT_DIR, "nfl_rushing_yards_props_tracking.json")
PLAYER_STATS_CACHE = os.path.join(SCRIPT_DIR, "nfl_player_rushing_yards_stats_cache.json")
TEAM_DEFENSE_CACHE = os.path.join(SCRIPT_DIR, "nfl_team_run_defense_cache.json")
...
def get_nfl_player_rushing_yards_stats():
    """
    Fetch REAL NFL player rushing yards stats from ESPN API
    Returns dictionary with player rushing yards stats (season avg, recent form, etc.)
    """
    print(f"\n{Colors.CYAN}Fetching REAL NFL player rushing yards statistics...{Colors.END}")
    ...
    try:
        # Fetch from ESPN API
        print(f"{Colors.CYAN}  Fetching season rushing yards stats from ESPN...{Colors.END}")
...
def get_opponent_run_defense_factors():
    """
    Fetch team run defense stats to identify matchup advantages
    Returns dict with opponent run defense factors (rushing yards allowed, YPC allowed)
    """
...
def fetch_player_rushing_yards_from_espn(player_name, team_name, game_date_str):
    """
    Fetch actual player rushing yards from ESPN API for a specific game
    Returns the actual rushing yards or None if not found
    """
    try:
                ...
                for stat_group in team_statistics:
                    ...
                    for athlete_data in athletes:
                        ...
                        if (len(name_parts) >= 2 and len(athlete_parts) >= 2 and
                            ...
                            # Get rushing yards stat
                            stats = athlete_data.get('stats', [])
                            for stat in stats:
                                if stat.get('name') == 'rushingYards' or stat.get('label') == 'RUSH YDS' or 'rush' in stat.get('name', '').lower() and 'yard' in stat.get('name', '').lower():
                                    rush_yds_value = stat.get('value', stat.get('displayValue', '0'))
                                    try:
                                        return int(float(str(rush_yds_value).replace(',', '')))
                                    except:
                                        pass
                            
                            # Try alternative stat format
                            for key, value in athlete_data.items():
                                if ('rush' in key.lower() and 'yard' in key.lower()) and isinstance(value, (int, float)):
                                    return int(value)
    ...
    for pick in pending_picks:
        try:
            ...
            if hours_ago > 4:
                ...
                # Fetch actual rushing yards
                actual_rush_yds = fetch_player_rushing_yards_from_espn(player_name, team_name, game_date_str)
                
                if actual_rush_yds is None:
                ...
                if bet_type == 'over':
                    is_win = actual_rush_yds > prop_line
                else:  # under
                    is_win = actual_rush_yds < prop_line
                ...
                pick['actual_rush_yds'] = actual_rush_yds
                ...
                print(f"    {result_str}: {player_name} had {actual_rush_yds} rushing yards (line: {prop_line}, bet: {bet_type.upper()})")
                updated += 1
...
def get_player_props():
    """Fetch player rushing yards prop odds from The Odds API"""
    print(f"\n{Colors.CYAN}Fetching player rushing yards prop odds...{Colors.END}")
                ...
                if 'bookmakers' in odds_data and odds_data['bookmakers']:
                    ...
                    if 'markets' in fanduel:
                        for market in fanduel['markets']:
                            if market['key'] == 'player_rushing_yards':
...
def calculate_ai_score(player_data, prop_line, bet_type, opponent_defense=None):
    """
    Calculate STRICT A.I. Score for rushing yards props using REAL stats
    Factors: Season avg, recent form, carry share, opponent run defense
    """
    score = 4.0

    season_avg = player_data.get('season_rush_yds_avg', 0)
    recent_avg = player_data.get('recent_rush_yds_avg', 0)
    carry_share = player_data.get('carry_share', 0.15)
    ...
    if bet_type == 'over':
        ...
        if carry_share >= 0.25:
            score += 1.5
        elif carry_share >= 0.20:
            score += 1.0
        elif carry_share >= 0.15:
            score += 0.5
        ...
        # Opponent factors
        if opponent_defense:
            def_factor = opponent_defense.get('rushing_yards_factor', 1.0)
            if def_factor > 1.05:
                score += 1.0
    ...
    for prop in props_list:
        ...
        if over_score >= MIN_AI_SCORE:
            ...
            if season_avg >= prop_line + 5.0 and recent_avg >= prop_line + 3.0:
                ...
                over_plays.append({
                    'player': player_name,
                    'prop': f"OVER {prop_line} RUSH YDS",
                    'team': prop['team'],
        ...
        if under_score >= MIN_AI_SCORE:
            ...
            if season_avg <= prop_line - 5.0 and recent_avg <= prop_line - 3.0:
                ...
                under_plays.append({
                    'player': player_name,
                    'prop': f"UNDER {prop_line} RUSH YDS",
                    'team': prop['team'],
                    'opponent': opponent_team,
...
<body>
    <div class="container">
        <div class="card header-card">
            ...
            <p style="font-size: 1.5rem; opacity: 0.95; font-weight: 600;">NFL Rushing Yards Props Model</p>
            <div>
...
def main():
    ...
    print(f"{Colors.BOLD}{Colors.CYAN}NFL RUSHING YARDS PROPS A.I. MODEL{Colors.END}")
    print(f"{Colors.BOLD}{Colors.CYAN}{'='*80}{Colors.END}")

    updated = update_pick_results()
    player_stats = get_nfl_player_rushing_yards_stats()
    defense_factors = get_opponent_run_defense_factors()
