#!/usr/bin/env python3
"""
NFL Receiving Yards Props Model - PROFITABLE VERSION
Analyzes player receiving yards props using REAL NFL stats
...
OUTPUT_HTML = os.path.join(SCRIPT_DIR, "nfl_receiving_yards_props.html")
TRACKING_FILE = os.path.join(SCRIPT_DIR, "nfl_receiving_yards_props_tracking.json")
PLAYER_STATS_CACHE = os.path.join(SCRIPT_DIR, "nfl_player_receiving_yards_stats_cache.json")
...
# Edge requirements - Adjusted for receiving yards (yards are larger numbers)
MIN_EDGE_OVER_LINE = 12.0  # Player must average 12+ above prop line for OVER
MIN_EDGE_UNDER_LINE = 10.0  # Player must average 10+ below prop line for UNDER
MIN_RECENT_FORM_EDGE = 8.0  # Recent form must strongly support
...
def get_nfl_player_receiving_yards_stats():
    """
    Fetch REAL NFL player receiving yards stats from ESPN API
    Returns dictionary with player receiving yards stats (season avg, recent form, etc.)
    """
    print(f"\n{Colors.CYAN}Fetching REAL NFL player receiving yards statistics...{Colors.END}")
    ...
    try:
        # Fetch from ESPN API
        print(f"{Colors.CYAN}  Fetching season receiving yards stats from ESPN...{Colors.END}")
...
def fetch_player_receiving_yards_from_espn(player_name, team_name, game_date_str):
    """
    Fetch actual player receiving yards from ESPN API for a specific game
    Returns the actual receiving yards or None if not found
    """
    try:
                ...
                for stat_group in team_statistics:
                    ...
                    for athlete_data in athletes:
                        ...
                        if (len(name_parts) >= 2 and len(athlete_parts) >= 2 and
                            ...
                            # Get receiving yards stat
                            stats = athlete_data.get('stats', [])
                            for stat in stats:
                                if stat.get('name') == 'receivingYards' or stat.get('label') == 'REC YDS' or 'receiving' in stat.get('name', '').lower() and 'yard' in stat.get('name', '').lower():
                                    rec_yds_value = stat.get('value', stat.get('displayValue', '0'))
                                    try:
                                        return int(float(str(rec_yds_value).replace(',', '')))
                                    except:
                                        pass
                            
                            # Try alternative stat format
                            for key, value in athlete_data.items():
                                if ('receiving' in key.lower() or 'rec' in key.lower()) and 'yard' in key.lower() and isinstance(value, (int, float)):
                                    return int(value)
    ...
    for pick in pending_picks:
        try:
            ...
            if hours_ago > 4:
                ...
                # Fetch actual receiving yards
                actual_rec_yds = fetch_player_receiving_yards_from_espn(player_name, team_name, game_date_str)
                
                if actual_rec_yds is None:
                ...
                if bet_type == 'over':
                    is_win = actual_rec_yds > prop_line
                else:  # under
                    is_win = actual_rec_yds < prop_line
                ...
                pick['actual_rec_yds'] = actual_rec_yds
                ...
                print(f"    {result_str}: {player_name} had {actual_rec_yds} receiving yards (line: {prop_line}, bet: {bet_type.upper()})")
                updated += 1
...
def get_player_props():
    """Fetch player receiving yards prop odds from The Odds API"""
    print(f"\n{Colors.CYAN}Fetching player receiving yards prop odds...{Colors.END}")
...
def calculate_ai_score(player_data, prop_line, bet_type, opponent_defense=None):
    """
    Calculate STRICT A.I. Score for receiving yards props using REAL stats
    Factors: Season avg, recent form, target share, opponent pass defense
    """
    score = 4.0

    season_avg = player_data.get('season_rec_yds_avg', 0)
    recent_avg = player_data.get('recent_rec_yds_avg', 0)
    ...
    if bet_type == 'over':
        ...
        # Opponent factors
        if opponent_defense:
            def_factor = opponent_defense.get('receiving_yards_factor', 1.0)
            if def_factor > 1.05:
                score += 1.0
    ...
    for prop in props_list:
        ...
        if under_score >= MIN_AI_SCORE:
            ...
            if season_avg <= prop_line - 3.0 and recent_avg <= prop_line - 2.0:
                ...
                under_plays.append({
                    'player': player_name,
                    'prop': f"UNDER {prop_line} REC YDS",
                    'team': prop['team'],
                    'opponent': opponent_team,
...
<body>
    <div class="container">
        <div class="card header-card">
            ...
            <p style="font-size: 1.5rem; opacity: 0.95; font-weight: 600;">NFL Receiving Yards Props Model</p>
            <div>
...
def main():
    ...
    print(f"{Colors.BOLD}{Colors.CYAN}NFL RECEIVING YARDS PROPS A.I. MODEL{Colors.END}")
    print(f"{Colors.BOLD}{Colors.CYAN}{'='*80}{Colors.END}")

    updated = update_pick_results()
    player_stats = get_nfl_player_receiving_yards_stats()
