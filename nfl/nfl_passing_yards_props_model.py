#!/usr/bin/env python3
"""
NFL Passing Yards Props Model - PROFITABLE VERSION
Analyzes player passing yards props using REAL NFL stats
...
# Edge requirements - Strict for passing yards
MIN_EDGE_OVER_LINE = 25.0  # Player must average 25+ above prop line for OVER
MIN_EDGE_UNDER_LINE = 20.0  # Player must average 20+ below prop line for UNDER
MIN_RECENT_FORM_EDGE = 15.0  # Recent form must strongly support
...
def get_nfl_player_passing_yards_stats():
    """
    Fetch REAL NFL player passing yards stats from ESPN API
    Returns dictionary with player passing yards stats (season avg, recent form, etc.)
    """
    print(f"\n{Colors.CYAN}Fetching REAL NFL player passing yards statistics...{Colors.END}")
    ...
    try:
        # Fetch from ESPN API
        print(f"{Colors.CYAN}  Fetching season passing yards stats from ESPN...{Colors.END}")
...
def fetch_player_passing_yards_from_espn(player_name, team_name, game_date_str):
    """
    Fetch actual player passing yards from ESPN API for a specific game
    Returns the actual passing yards or None if not found
    """
    try:
                ...
                for stat_group in team_statistics:
                    ...
                    for athlete_data in athletes:
                        ...
                        if (len(name_parts) >= 2 and len(athlete_parts) >= 2 and
                            ...
                            # Get passing yards stat
                            stats = athlete_data.get('stats', [])
                            for stat in stats:
                                if stat.get('name') == 'passingYards' or stat.get('label') == 'PASS YDS' or 'passing' in stat.get('name', '').lower() and 'yard' in stat.get('name', '').lower():
                                    pass_yds_value = stat.get('value', stat.get('displayValue', '0'))
                                    try:
                                        return int(float(str(pass_yds_value).replace(',', '')))
                                    except:
                                        pass
                            
                            # Try alternative stat format
                            for key, value in athlete_data.items():
                                if ('passing' in key.lower() or 'pass' in key.lower()) and 'yard' in key.lower() and isinstance(value, (int, float)):
                                    return int(value)
    ...
    for pick in pending_picks:
        try:
            ...
            if hours_ago > 4:
                ...
                # Fetch actual passing yards
                actual_pass_yds = fetch_player_passing_yards_from_espn(player_name, team_name, game_date_str)
                
                if actual_pass_yds is None:
                ...
                if bet_type == 'over':
                    is_win = actual_pass_yds > prop_line
                else:  # under
                    is_win = actual_pass_yds < prop_line
                ...
                pick['actual_pass_yds'] = actual_pass_yds
                ...
                print(f"    {result_str}: {player_name} had {actual_pass_yds} passing yards (line: {prop_line}, bet: {bet_type.upper()})")
                updated += 1
...
def get_player_props():
    """Fetch player passing yards prop odds from The Odds API"""
    print(f"\n{Colors.CYAN}Fetching player passing yards prop odds...{Colors.END}")
...
def calculate_ai_score(player_data, prop_line, bet_type, opponent_defense=None):
    """
    Calculate STRICT A.I. Score for passing yards props using REAL stats
    Factors: Season avg, recent form, pass volume, opponent pass defense
    """
    score = 4.0

    season_avg = player_data.get('season_pass_yds_avg', 0)
    recent_avg = player_data.get('recent_pass_yds_avg', 0)
    pass_attempts = player_data.get('pass_attempts', 0)
    ...
    if bet_type == 'over':
        ...
        if pass_attempts >= 35:
            score += 1.5
        elif pass_attempts >= 30:
            score += 1.0
        elif pass_attempts >= 25:
            score += 0.5
        ...
        # Opponent factors
        if opponent_defense:
            def_factor = opponent_defense.get('passing_yards_factor', 1.0)
            if def_factor > 1.05:
                score += 1.0
...
def analyze_props(props_list, player_stats, defense_factors):
    ...
    for prop in props_list:
        ...
        if over_score >= MIN_AI_SCORE:
            ...
            if season_avg >= prop_line + 10.0 and recent_avg >= prop_line + 5.0:
                # Calculate EV
                ...
                over_plays.append({
                    'player': player_name,
                    'prop': f"OVER {prop_line} PASS YDS",
                    'team': prop['team'],
        ...
        if under_score >= MIN_AI_SCORE:
            ...
            if season_avg <= prop_line - 10.0 and recent_avg <= prop_line - 5.0:
                ...
                under_plays.append({
                    'player': player_name,
                    'prop': f"UNDER {prop_line} PASS YDS",
                    'team': prop['team'],
                    'opponent': opponent_team,
...
<body>
    <div class="container">
        <div class="card header-card">
            ...
            <p style="font-size: 1.5rem; opacity: 0.95; font-weight: 600;">NFL Passing Yards Props Model</p>
            <div>
