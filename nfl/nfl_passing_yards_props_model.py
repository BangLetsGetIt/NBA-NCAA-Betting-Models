...
def calculate_probability_edge(ai_score, season_avg, recent_avg, prop_line, odds, bet_type):
    ...
    # For passing yards, normalize edge differently (25 yards = full edge factor)
    edge_factor = min(abs(edge) / 25.0, 1.0)
    
    recent_factor = 0.0
    if bet_type == 'over' and recent_avg > season_avg:
        recent_factor = min((recent_avg - season_avg) / 25.0, 0.1)
    elif bet_type == 'under' and recent_avg < season_avg:
        recent_factor = min((season_avg - recent_avg) / 25.0, 0.1)
...
def calculate_ai_score(player_data, prop_line, bet_type, opponent_defense=None):
    ...
    Factors: Season avg, recent form, pass attempts, opponent pass defense
    """
    score = 4.0
    ...
    if pass_attempts < 20:  # Not enough volume
        return 0.0
    ...
    if bet_type == 'over':
        ...
        if edge_above_line >= MIN_EDGE_OVER_LINE:
            score += 3.5
        elif edge_above_line >= 15.0:
            score += 2.5
        elif edge_above_line >= 10.0:
            score += 1.5
        elif edge_above_line >= 5.0:
            score += 0.5
        else:
            score -= 2.0
            if recent_avg < prop_line + 10.0:
                return 0.0

        recent_edge = recent_avg - prop_line
        if recent_edge >= MIN_RECENT_FORM_EDGE:
            score += 2.5
        elif recent_edge >= 10.0:
            score += 1.5
        elif recent_avg > season_avg + 15.0:
            score += 1.0
        ...
        if edge_below_line >= MIN_EDGE_UNDER_LINE:
            score += 3.5
        elif edge_below_line >= 15.0:
            score += 2.5
        elif edge_below_line >= 10.0:
            score += 1.5
        elif edge_below_line >= 5.0:
            score += 0.5
        else:
            score -= 2.0
            if recent_avg > prop_line - 10.0:
    ...
    if bet_type == 'over' and season_avg < prop_line + 10.0:
        final_score = min(final_score, 8.5)
    elif bet_type == 'under' and season_avg > prop_line - 10.0:
        final_score = min(final_score, 8.5)
...
def calculate_ev(ai_score, prop_line, season_avg, recent_avg, odds, bet_type):
    ...
    # Normalize edge for passing yards (25 yards = full factor)
    edge_factor = min(abs(edge) / 25.0, 1.0)
...
<head>
    ...
    <title>NFL Passing Yards Props - A.I. Projections</title>
    <style>
