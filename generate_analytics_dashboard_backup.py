...
def load_all_tracking_data(self):
                    ...
                    with open(full_path, 'r') as f:
                        ...
                        if isinstance(picks, list):
                            for pick in picks:
                                ...
                                pick['model_name'] = self.get_model_name_from_file(file_path)
    ...
    def get_model_name_from_file(self, file_path):
        """Get model name for display in analytics"""
        model_mapping = {
            ...
            'best_plays_tracking.json': 'Best Plays'
        }
        return model_mapping.get(file_path.replace('.json', ''), 'Unknown Model')
...
# NBA team abbreviation mapping
    ...
    def calculate_model_breakdown(self):
        """Calculate model breakdown with recent performance"""
        model_stats = {}
        ...
        model_mapping = {
            ...
            'nba/nba_assists_props_tracking.json': 'NBA Assists Props',
            'nba/nba_3pt_props_tracking.json': 'NBA 3PT Props',
            'nfl/nfl_picks_tracking.json': 'NFL Main Model',
            'nfl/nfl_passing_yards_props_tracking.json': 'NFL Passing Yards',
            'nfl/nfl_rushing_yards_props_tracking.json': 'NFL Rushing Yards',
            'nfl/nfl_receiving_yards_props_tracking.json': 'NFL Receiving Yards',
            'nfl/nfl_receptions_props_tracking.json': 'NFL Receptions',
            'nfl/atd_model_tracking.json': 'NFL ATD Model',
            'ncaa/ncaab_picks_tracking.json': 'NCAAB Main Model',
            'ncaa/cbb_rebounds_props_tracking.json': 'NCAAB Rebounds Props',
            'soccer/soccer_picks_tracking.json': 'Soccer Model',
            'wnba/wnba_model_tracking.json': 'WNBA Main Model',
