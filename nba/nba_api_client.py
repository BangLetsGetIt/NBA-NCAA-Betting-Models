...
# Constants
CACHE_FILE = os.path.join(os.path.dirname(os.path.abspath(__file__)), "nba_odds_cache.json")
CACHE_DURATION_MINUTES = 45  # Increased to 45 mins to be safe
...
class NBAOddsClient:
    ...
    def get_odds_data(self, markets=None):
        ...
        if cached_data:
            print(f"  ✓ Using cached odds data ({self._get_cache_age(cached_data)} mins old)")
            return cached_data
            
        # Fetch from API
        print(f"  ⚡ Cache stale or missing. Fetching fresh data from API...")
    ...
    def _fetch_from_api(self, markets):
        ...
        print(f"  found {len(upcoming_events)} upcoming games")
        
        # 2. Fetch Odds for each game
        # We limit to 10 games to save quota as per original logic, 
        # or we could fetch all if quota allows. Sticking to 10 for safety.
        games_data = []
    ...
    def _save_cache(self, games_data):
        ...
        try:
            ...
            print(f"  ✓ Saved {len(games_data)} games to cache: {CACHE_FILE}")
        except Exception as e:
