#!/usr/bin/env python3
"""
NBA Rebounds Props Model - PROFITABLE VERSION
Analyzes player rebounds props using REAL NBA stats
...
OUTPUT_HTML = os.path.join(SCRIPT_DIR, "nba_rebounds_props.html")
TRACKING_FILE = os.path.join(SCRIPT_DIR, "nba_rebounds_props_tracking.json")
PLAYER_STATS_CACHE = os.path.join(SCRIPT_DIR, "nba_player_rebounds_stats_cache.json")
TEAM_REBOUNDING_CACHE = os.path.join(SCRIPT_DIR, "nba_team_rebounding_cache.json")
...
# Edge requirements - STRICTER for rebounds (more variance)
...
def get_nba_player_rebounds_stats():
    """
    Fetch REAL NBA player rebounding stats from NBA API
    Returns dictionary with player rebounding stats (season avg, recent form, etc.)
    """
    print(f"\n{Colors.CYAN}Fetching REAL NBA player rebounding statistics...{Colors.END}")
    ...
    try:
        # Fetch season stats
        print(f"{Colors.CYAN}  Fetching season rebounding stats...{Colors.END}")
        ...
        for _, row in season_df.iterrows():
            ...
            # Season averages
            season_reb = row.get('REB', 0)  # Total rebounds per game
            season_oreb = row.get('OREB', 0)  # Offensive rebounds
            season_dreb = row.get('DREB', 0)  # Defensive rebounds
            ...
            if not recent_row.empty:
                recent_reb = recent_row.iloc[0].get('REB', season_reb)
                recent_oreb = recent_row.iloc[0].get('OREB', season_oreb)
                recent_dreb = recent_row.iloc[0].get('DREB', season_dreb)
            else:
                recent_reb = season_reb
                recent_oreb = season_oreb
                recent_dreb = season_dreb

            # Calculate consistency (rebounds per 36 minutes)
            reb_per_36 = (season_reb / minutes * 36) if minutes > 0 else 0
            consistency = min(1.0, reb_per_36 / 12.0) if reb_per_36 > 0 else 0.3  # Normalize to 12 reb/36

            player_stats[player_name] = {
                'season_reb_avg': round(season_reb, 2),
                'season_oreb_avg': round(season_oreb, 2),
                'season_dreb_avg': round(season_dreb, 2),
                'recent_reb_avg': round(recent_reb, 2),
                'recent_oreb_avg': round(recent_oreb, 2),
                'recent_dreb_avg': round(recent_dreb, 2),
...
def get_opponent_rebounding_factors():
    """
    Fetch team rebounding stats to identify matchup advantages
    Returns dict with opponent rebounding factors (offensive reb%, defensive reb%, pace)
    """
    print(f"\n{Colors.CYAN}Fetching opponent rebounding factors...{Colors.END}")

    # Check cache
    if os.path.exists(TEAM_REBOUNDING_CACHE):
        file_mod_time = datetime.fromtimestamp(os.path.getmtime(TEAM_REBOUNDING_CACHE))
        if (datetime.now() - file_mod_time) < timedelta(hours=6):
            print(f"{Colors.GREEN}✓ Using cached rebounding factors{Colors.END}")
            with open(TEAM_REBOUNDING_CACHE, 'r') as f:
                return json.load(f)
    ...
    try:
        ...
        # Process team rebounding stats
        for _, row in team_df.iterrows():
            team_name = row.get('TEAM_NAME', '')
            if not team_name:
                continue

            # Opponent rebounding stats (what they allow)
            opp_oreb = row.get('OPP_OREB', 0)  # Opponent offensive rebounds allowed
            opp_dreb = row.get('OPP_DREB', 0)  # Opponent defensive rebounds allowed
            opp_tot_reb = opp_oreb + opp_dreb
            
            # Team pace
            pace = row.get('PACE', 100)
            
            # Opponent shooting % (lower = more misses = more rebounds available)
            opp_fg_pct = row.get('OPP_FG_PCT', 0.45)
            
            # Calculate rebounding advantage factors
            # Low opponent offensive reb% = more defensive rebounds available
            # Low opponent shooting% = more misses = more rebounds
            # High pace = more possessions = more rebounds
            
            rebounding_factors[team_name] = {
                'opp_oreb_allowed': round(opp_oreb, 2),
                'opp_dreb_allowed': round(opp_dreb, 2),
                'opp_tot_reb_allowed': round(opp_tot_reb, 2),
                'pace': round(pace, 2),
                'opp_fg_pct': round(opp_fg_pct, 3),
                'rebounding_factor': round((1.0 - opp_fg_pct) * (pace / 100), 2)  # Higher = better for rebounds
            }

        # Cache results
        with open(TEAM_REBOUNDING_CACHE, 'w') as f:
            json.dump(rebounding_factors, f, indent=2)

        print(f"{Colors.GREEN}✓ Fetched rebounding factors for {len(rebounding_factors)} teams{Colors.END}")
        return rebounding_factors

    except Exception as e:
        print(f"{Colors.YELLOW}⚠ Could not fetch rebounding factors: {e}{Colors.END}")
        # Try cache
        if os.path.exists(TEAM_REBOUNDING_CACHE):
            with open(TEAM_REBOUNDING_CACHE, 'r') as f:
                return json.load(f)
...
def fetch_player_rebounds_from_nba_api(player_name, team_name, game_date_str):
    ...
    try:
        ...
        print(f"{Colors.YELLOW}  Error fetching rebounds from NBA API for {player_name}: {str(e)}{Colors.END}")
        return None
    ...
    for pick in pending_picks:
        try:
            ...
            if hours_ago > 4:
                ...
                # Fetch actual rebounds
                actual_rebounds = fetch_player_rebounds_from_nba_api(player_name, team_name, game_date_str)
                ...
                # Determine win/loss
                # Convert to float to ensure correct numeric comparison (not string comparison)
                prop_line = float(pick['prop_line'])
                actual_rebounds = float(actual_rebounds)
                bet_type = pick['bet_type'].lower()
                
                # Logic: OVER wins if actual > line, UNDER wins if actual < line
                # Example: Line 8.5, actual 9 = OVER wins, UNDER loses
                # Example: Line 8.5, actual 8 = OVER loses, UNDER wins
                if bet_type == 'over':
                    is_win = actual_rebounds > prop_line
                else:  # under
                    is_win = actual_rebounds < prop_line
                ...
                pick['actual_reb'] = actual_rebounds
                ...
                print(f"    {result_str}: {player_name} had {actual_rebounds} rebounds (line: {prop_line}, bet: {bet_type.upper()})")
                updated += 1
...
def calculate_ai_score(player_data, prop_line, bet_type, opponent_rebounding=None):
    """
    Calculate STRICT A.I. Score for rebounds props using REAL stats
    Factors: Season avg, recent form, pace, opponent rebounding factors, consistency
    """
    score = 4.0

    season_avg = player_data.get('season_reb_avg', 0)
    recent_avg = player_data.get('recent_reb_avg', 0)
    reb_per_36 = player_data.get('reb_per_36', 0)
    ...
    if bet_type == 'over':
        ...
        # Opponent factors (pace, shooting %, rebounding)
        if opponent_rebounding:
            reb_factor = opponent_rebounding.get('rebounding_factor', 1.0)
            if reb_factor > 1.05:
                score += 1.0
            elif reb_factor < 0.95:
                score -= 0.5
...
def analyze_props(props_list, player_stats, rebounding_factors, historical_edge_performance=None):
    ...
    for prop in props_list:
        ...
        opponent_rebounding = None
        if opponent_team in rebounding_factors:
            opponent_rebounding = rebounding_factors[opponent_team]
        else:
            for team_name, factors in rebounding_factors.items():
                if opponent_team.lower() in team_name.lower() or team_name.lower() in opponent_team.lower():
                    opponent_rebounding = factors
                    break

        over_score = calculate_ai_score(player_data, prop_line, 'over', opponent_rebounding)
        ...
        under_score = calculate_ai_score(player_data, prop_line, 'under', opponent_rebounding)
...
def generate_html_output(over_plays, under_plays, tracking_summary=None, tracking_data=None):
    ...
    if tracking_summary and tracking_summary['total'] > 0:
        ...
        if True:
            ...
            tracking_section = f"""
            <div class="card">
                <h2 style="font-size: 1.75rem; font-weight: 700; margin-bottom: 1.5rem; text-align: center;">📊 NBA Rebounds Model Tracking</h2>
    ...
    if over_plays:
        ...
        for i, play in enumerate(over_plays, 1):
            ...
            if play['team'] == home_team:
                # Player's team is home, so show: opponent @ team
                matchup_display = f"{short_opponent} @ {short_team}"
            else:
                # Player's team is away, so show: team @ opponent
...
def main():
    ...
    print(f"{Colors.BOLD}{Colors.CYAN}NBA REBOUNDS PROPS A.I. MODEL{Colors.END}")
    print(f"{Colors.BOLD}{Colors.CYAN}{'='*80}{Colors.END}")

    updated = update_pick_results()
    player_stats = get_nba_player_rebounds_stats()
    rebounding_factors = get_opponent_rebounding_factors()
    ...
    over_plays, under_plays = analyze_props(props_list, player_stats, rebounding_factors, historical_edge_performance)

    print(f"\n{Colors.CYAN}Auto-tracking picks with A.I. Score >= {AUTO_TRACK_THRESHOLD}...{Colors.END}")
    tracked_count = 0
    
    # Track ALL plays meeting threshold (preserve complete history)
    # displayed_plays is only used for filtering pending count in summary
