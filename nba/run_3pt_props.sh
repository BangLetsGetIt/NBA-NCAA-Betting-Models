...
# NBA 3PT Props Model Automation Script
# Runs the 3PT props model with error handling
...
LOG_FILE="logs/nba_3pt_props_$(date +%Y%m%d_%H%M%S).log"

echo "==========================================" | tee "$LOG_FILE"
echo "NBA 3PT Props Model Run: $(date)" | tee -a "$LOG_FILE"
echo "==========================================" | tee -a "$LOG_FILE"
...
# Run the model
echo "Running NBA 3PT Props model..." | tee -a "$LOG_FILE"
python3 nba_3pt_props_model.py 2>&1 | tee -a "$LOG_FILE"
