...
# NBA Rebounds Props Model Automation Script
# Runs the rebounds props model with error handling
...
LOG_FILE="logs/nba_rebounds_props_$(date +%Y%m%d_%H%M%S).log"

echo "==========================================" | tee "$LOG_FILE"
echo "NBA Rebounds Props Model Run: $(date)" | tee -a "$LOG_FILE"
echo "==========================================" | tee -a "$LOG_FILE"
...
# Run the model
echo "Running NBA Rebounds Props model..." | tee -a "$LOG_FILE"
python3 nba_rebounds_props_model.py 2>&1 | tee -a "$LOG_FILE"
