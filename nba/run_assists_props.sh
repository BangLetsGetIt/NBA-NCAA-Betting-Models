...
# NBA Assists Props Model Automation Script
# Runs the assists props model with error handling
...
LOG_FILE="logs/nba_assists_props_$(date +%Y%m%d_%H%M%S).log"

echo "==========================================" | tee "$LOG_FILE"
echo "NBA Assists Props Model Run: $(date)" | tee -a "$LOG_FILE"
echo "==========================================" | tee -a "$LOG_FILE"
...
# Run the model
echo "Running NBA Assists Props model..." | tee -a "$LOG_FILE"
python3 nba_assists_props_model.py 2>&1 | tee -a "$LOG_FILE"
